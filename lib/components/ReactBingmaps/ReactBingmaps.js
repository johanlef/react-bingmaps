...
var scriptURL = 'https://www.bing.com/api/maps/mapcontrol?callback=bingmapsCallback';
  ...
  _createClass(ReactBingmaps, [{
    ...
    value: function setBoundary(boundary, mapReference) {
      if (map[mapReference] && boundary) {
        ...
        var errCount = {};
        var errCallback = function errCallback(boundaryLocation, statusMessage) {
          errCount[boundaryLocation] = errCount[boundaryLocation] ? errCount[boundaryLocation] + 1 : 0;
          console.debug(boundaryLocation, statusMessage, errCount);
          if (errCount[boundaryLocation] < 5) loadBoundaries();
        };
        var loadBoundaries = function loadBoundaries() {
