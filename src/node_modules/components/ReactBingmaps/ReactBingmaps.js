...
class ReactBingmaps extends Component {
  ...
  setBoundary (boundary, mapReference) {
    if (map[mapReference] && boundary) {
      ...
      var errCount = {}
      var errCallback = function (boundaryLocation, statusMessage) {
        errCount[boundaryLocation] = errCount[boundaryLocation]
          ? errCount[boundaryLocation] + 1
          : 0
        console.debug(boundaryLocation, statusMessage, errCount)
        if (errCount[boundaryLocation] < 5) loadBoundaries()
      }
      var loadBoundaries = function () {
